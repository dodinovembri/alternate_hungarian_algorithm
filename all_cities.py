...
for first_array in data_transpose:
    ...
    for second_array in first_array:
        first_value = second_array[0]
        second_value = second_array[1]
        expand_value = []
        fuzzification = (second_value - first_value) / 7
        integer_of_fuzzification = int(fuzzification)
        if(integer_of_fuzzification == 0):
            for i in range(first_value, second_value+1):
                expand_value.append(i)
        else:
            for i in range(first_value, second_value+1, integer_of_fuzzification):
                expand_value.append(i)
...
robust_value = []
crisp_value = []
for robust_first_array in relevant_value:
    robust_data_pushed = []
    crisp_value_pushed = []
    for robust_second_array in robust_first_array:
        robust_first_value = robust_second_array[0]
        robust_second_value = robust_second_array[1]
        fuzzification = (robust_second_value - robust_first_value) / 7
        integer_of_fuzzification = int(fuzzification)
        sum_range = 0
        if(integer_of_fuzzification == 0):
            for j in range(robust_first_value, robust_second_value+1):
                sum_range = sum_range + j
        else:
            for j in range(robust_first_value, robust_second_value+1, integer_of_fuzzification):
                sum_range = sum_range + j
        robust = 1/4 * sum_range
        robust_data = [robust_second_array, [robust]]
        crisp_value_each = robust
        crisp_value_pushed.append(crisp_value_each)
        robust_data_pushed.append(robust_data)
    robust_value.append(robust_data_pushed)
    crisp_value.append(crisp_value_pushed)
robust_value_transpose = [list(i) for i in zip(*crisp_value)]
data_route_push = [list(i) for i in zip(*crisp_value)]
